// // function imprimeNome(nome){
// //     console.log("Olá", nome)
// // }
// const nome = prompt("Digite seu nome")
//  imprimeNome(nome)

//  function imprimeTabuada(numero){
//     console.log("1 *" , numero , "=" , 1 * numero)
//     console.log("2 *" , numero , "=" , 2 * numero)
//     console.log("3 *" , numero , "=" , 3 * numero)
//     console.log("4 *" , numero , "=" , 4 * numero)
//     console.log("5 *" , numero , "=" , 5 * numero)
//     console.log("6 *" , numero , "=" , 6 * numero)
//     console.log("7 *" , numero , "=" , 7 * numero)
//     console.log("8 *" , numero , "=" , 8 * numero)
//     console.log("9 *" , numero , "=" , 9 * numero)
//     console.log("10 *" , numero , "=" , 10 * numero)
//  }



    const imprimeNome = (nome) =>{
        console.log("Olá", nome)
    }
    const nome = prompt("Digite seu nome")
    imprimeNome(nome)

    const imprimeTabuada = (numero) => {
        console.log("1 *" , numero , "=" , 1 * numero)
        console.log("2 *" , numero , "=" , 2 * numero)
        console.log("3 *" , numero , "=" , 3 * numero)
        console.log("4 *" , numero , "=" , 4 * numero)
        console.log("5 *" , numero , "=" , 5 * numero)
        console.log("6 *" , numero , "=" , 6 * numero)
        console.log("7 *" , numero , "=" , 7 * numero)
        console.log("8 *" , numero , "=" , 8 * numero)
        console.log("9 *" , numero , "=" , 9 * numero)
        console.log("10 *" , numero , "=" , 10 * numero)
    }

    imprimeTabuada(4)

    


    

    
 






